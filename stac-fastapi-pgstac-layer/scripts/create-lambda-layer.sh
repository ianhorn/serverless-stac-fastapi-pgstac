...
find $PREFIX/python -type f -name '*.pyc' | while read f; do
...
cd $PREFIX
zip -r9q /tmp/package.zip python
zip -r9q /tmp/package.zip lib

echo "Copy package to mounted volume"

mkdir -p /local

ls -lah /tmp/package.zip
ls -lah /local
