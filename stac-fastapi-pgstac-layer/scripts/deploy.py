...
def main(runtime, version):
    ...
    click.echo("Deploying stac-fastapi-pgstac layer", err=True)
    for region in AWS_REGIONS:
        ...
        s3_bucket = f"stac-fastapi-pgstac-layers-{region}"
        s3_key = f"stac-fastapi-pgstac{version_nodot}-py{runtime_nodot}.zip"
        ...
        res = lambda_client.publish_layer_version(
            LayerName="stac-fastapi-pgstac",
            Content={"S3Bucket": s3_bucket, "S3Key": s3_key},
            CompatibleRuntimes=[f"python{runtime}"],
            Description=f"TiTiler PgSTAC Lambda Layer ({version}) - for Python {runtime}",
            LicenseInfo="MIT",
        )
